...
namespace khronos {
	...
	jd_t Julian::to_jd() const {
		return julian_to_jd(year_, month_, day_, hour_, minute_, second_);
	}

	Julian& Julian::operator=(jd_t jd) {
		from_jd(jd);
		return *this;
	}

	Julian& Julian::operator+=(detail::packaged_year_real const& year) {
		long long whole_years = static_cast<long long>(year.value);
		year_ += whole_years;

		if (month_ == 2 && day_ == 29 && !is_julian_leapyear(year_)) {
			day_ = 28;
		}
	...
	Julian& Julian::operator+=(detail::packaged_month_integer const& month) {
		...
		int max_day = julian_days_in_month(month_, is_julian_leapyear(year_));
		if (day_ > max_day) {
			day_ = max_day;
		}
	...
	Julian Julian::operator+(detail::packaged_year_real const& year) const {
		Julian result(*this);
		result += year;
		return result;
	}

	Julian Julian::operator+(detail::packaged_month_integer const& month) const {
		Julian result(*this);
		result += month;
		return result;
	}
