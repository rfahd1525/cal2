...
namespace khronos {
	...
	Vulcan::Vulcan(jd_t jd) {
		from_jd(jd);
	}

	Vulcan::Vulcan(year_t year, month_t month, day_t day)
	...
	Vulcan& Vulcan::operator=(jd_t jd) {
		from_jd(jd);
		return *this;
	}

	Vulcan& Vulcan::operator+=(detail::packaged_year_real const& year) {
		long long whole_years = static_cast<long long>(year.value);
		year_ += whole_years;

		if (month_ == 12 && day_ == 22 && !is_vulcan_leapyear(year_)) {
			day_ = 21;
		}
	...
	Vulcan& Vulcan::operator+=(detail::packaged_month_integer const& month) {
		...
		int max_day = vulcan_days_in_month(month_, is_vulcan_leapyear(year_));
		if (day_ > max_day) {
			day_ = max_day;
		}
	...
	Vulcan Vulcan::operator+(detail::packaged_year_real const& year) const {
		Vulcan result(*this);
		result += year;
		return result;
	}

	Vulcan Vulcan::operator+(detail::packaged_month_integer const& month) const {
		Vulcan result(*this);
		result += month;
		return result;
	}
